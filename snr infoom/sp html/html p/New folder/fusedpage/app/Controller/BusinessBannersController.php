...
class BusinessBannersController extends AppController {
	...
	public function admin_manage($id=null){
		...
		if($id != ''){
			...
			$businessArr = $this->BusinessBanner->find('first', array('conditions'=>array('BusinessBanner.business_id'=>$this->params['pass'][0]))); //pr($businessArr);die;

			if(!empty($businessArr)){
				foreach($businessArr['BusinessBanner'] as $key => $value){ //echo $key .'=>'. $value;die;
					if(substr($key, 0, 7) == 'banner_'){
						$ret[]['BusinessBanner']['banner'] = $value;
					}
				}
				$businessArr = $ret;
			}else{
