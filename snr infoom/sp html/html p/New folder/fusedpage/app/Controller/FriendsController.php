...
class FriendsController extends AppController {
	...
	function received_requests(){
		$user_id = $this->Session->read('Auth.User.User.id');
		$this->set('viewListing', $this->Friend->find('all', array('conditions'=>array('Friend.request_received'=>$user_id, 'Friend.friendship_status'=>'0'), 'order'=>array('Friend.id'=>'DESC'))));
	}
	...
	//FUNCTION FOR DELELTING A FRIEND REQUEST START
	public function delete_friend_request(){ //pr($_POST);
		$this->layout = false;

		if($this->Friend->delete($_POST['id']))
			$ret = 'deleted';
		else
			$ret = 'error';
		echo $ret;
	...
	//FUNCTION FOR DELELTING A FRIEND REQUEST END
	...
	//FUNCTION FOR UN-FRIEND OTHER PROFILE REQUEST START(SAURABH 5/3/2013)
	function unfriendother(){ //pr($_POST); die;
		$this->layout = 'ajax';
		$id = $_POST['id'];

		$or = array('0'=>array('Friend.request_sent'=>$this->Session->read('Auth.User.User.id'), 'Friend.request_received'=>$id), '1'=>array('Friend.request_sent'=>$id, 'Friend.request_received'=>$this->Session->read('Auth.User.User.id')));

		$this->Friend->recursive = -1;
		$frndsArr = $this->Friend->find('first', array('conditions'=>array('OR'=>$or, 'Friend.friendship_status'=>'1'))); //pr($frndsArr);die;
		$frndid = $frndsArr['Friend']['id']; 
		$this->Friend->delete($frndid);
		echo $return ='<span>'."Add as Friend".'</span>';
		exit;
	}
	//FUNCTION FOR UN-FRIEND REQUEST END(SAURABH 5/3/2013)
	...
	//FUNCTION FOR OTHER PROFILE STATUS MANAGEMENT STRAT
	public function other_profile_data(){ //pr($_POST);die;
		$this->layout = 'ajax';
		$type = $_POST['type'];
		$id = $_POST['id'];
		...
		if(($type == 'unfriend') || ($type == 'delete')){ // delete the record
			...
			$saveData['id'] = $id;
			$saveData['friendship_status'] = '1';
			if($this->Friend->save($saveData, false));
		}elseif($type == 'ignore'){ //for frnd rqst ignore
			$saveData['id'] = $id;
			$saveData['friendship_status'] = '2';
			$this->Friend->save($saveData, false);
		}elseif($type == 'add'){ //for frnd rqst ignore
			$saveData['request_sent'] = $this->Session->read('Auth.User.User.id');
			$saveData['request_received'] = $id;
			$saveData['friendship_status'] = '0';
			$this->Friend->save($saveData, false);
		}
		$this->set('type', $type);
	}
	//FUNCTION FOR OTHER PROFILE STATUS MANAGEMENT END
