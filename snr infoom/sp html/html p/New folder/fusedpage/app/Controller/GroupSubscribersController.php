		...
		if($_POST['type'] == 'subscribe'){ //subscribe business
			if(empty($subscribeArr)){
				...
				if($this->GroupSubscriber->save($saveData)){
					...
					$businessArr = $this->Group->find('first', array('fields'=>array('Group.title'), 'conditions'=>array('Group.id'=>$_POST['group_id'])));
					$link = SITE_PATH.'groups/details/'.$this->Fp->encrypt($_POST['group_id']).'/'.$this->Fp->parseParameterNew($businessArr['Group']['title']).'/';
					$content = $this->Session->read('Auth.User.User.first_name').' has subscribed '.$businessArr['Group']['title'].' on fusedpage '.$link;
