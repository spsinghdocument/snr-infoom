...
class GroupsController extends AppController {
	...
	function add_group_recommend(){ //pr($_POST); die;
		...
		$this->Group->recursive = -1;
		$groupArr = $this->Group->find('first', array('fields'=>array('Group.title'), 'conditions'=>array('Group.id'=>$_POST['group_id'])));
		
		$link = SITE_PATH.'groups/details/'.$this->Fp->encrypt($_POST['group_id']).'/'.$this->Fp->parseParameterNew($groupArr['Group']['title']).'/';
		$content = $this->Session->read('Auth.User.User.first_name').' has recommended '.$groupArr['Group']['title'].' on fusedpage '.$link;
