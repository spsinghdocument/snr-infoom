...
<div class="logofl">
	<?php echo $this->Html->link($this->Html->image('front_end/logo.jpg', array('alt'=>'', 'border'=>0)), $sitePath, array('escape'=>false));?>
</div>
...
<div class="logofr">
	<div class="fr">
		...
		<?php echo $this->Form->create('Business', array('action'=>'search'));?>
			<?php echo $this->Form->input('Business.keyword', array('onkeyup'=>'header_autosuggation(this.value);','div'=>false, 'label'=>false, 'error'=>false, 'type'=>'text', 'style'=>'width:315px;', 'class'=>'deshboartopinput', 'placeholder'=>'Find local businesses (i.e. caf&#233; or hotel or contractor, etc)')); ?>

			<div id="header_suggestions" style="border:1px solid #CDCDCD; width:315px; display:none; position:absolute; background-color:#FFFFFF; max-height:200px; overflow-x:hidden; overflow-y:scroll; z-index:10"></div>
			...
			<?php echo $this->Form->input('Business.city', array('onkeyup'=>'city_autosuggation(this.value);','div'=>false, 'label'=>false, 'error'=>false, 'type'=>'text', 'style'=>'width:100px;', 'class'=>'deshboartopinput', 'onfocus'=>'clearText(this)', 'onblur'=>'replaceText(this)', 'value'=>$this->Session->read('Auth.User.User.city'))); ?>

			<div id="city_suggestions" style="border:1px solid #CDCDCD; width:200px; margin-left:336px; display:none; position:absolute; background-color:#FFFFFF; max-height:200px; overflow-x:hidden; overflow-y:scroll; z-index:10"></div>
...
<?php if($this->Session->read('Auth.User.User.id') != ''){?>
		<div class="deshboardfrimgbox">
			...
			<div class="usernamefr">
				<?php echo $this->Html->link($this->Session->read('Auth.User.User.first_name'), SITE_PATH, array('escape'=>false, 'style'=>'text-decoration:none; color:#FFFFFF;'));?>
			</div>
			<div class="clr"></div>
						...
						<div class="userbtmshape">
							<div class="usermidbg">
								...
								<ul class="userpopuplist">
									<li><a href="<?php echo $sitePath.'users/settings/';?>">Account Settings</a></li>
									<li><a href="<?php echo $sitePath.'users/sign_out/';?>">Logout</a></li>
									<li class="last"><a href="<?php echo $sitePath.'faqs/faq/';?>">Help</a></li>
	...
	if(searchkey.length > 0){
		...
		if(searchkey != ''){
			$.ajax({
				...
				beforeSend:function(){
				//alert(categoryId);
					var bSend = '<?php echo $this->Html->image("ajax/small-facebook.gif",array("alt"=>"", "style"=>"margin-left:110px;"));?>'; 
...
function city_autosuggation(citykey){
	if(citykey.length > 0){
		$('#citybox').show();
		if(citykey != ''){
			$.ajax({
				type: "POST",
				url: "<?php echo SITE_PATH.'businesses/auto_city_data/';?>",
				data: "citykey="+citykey,
				beforeSend:function(){
				//alert(categoryId);
					var bSend = '<?php echo $this->Html->image("ajax/small-facebook.gif",array("alt"=>"", "style"=>"margin-left:110px;"));?>'; 
					$('#city_suggestions').show();
					$('#city_suggestions').html(bSend);
				},
				success: function(response){
					$('#city_suggestions').html(response);
				}
			});
		}
	}else{
