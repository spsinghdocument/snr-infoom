...
<div class="usermidtopbox">
	<ul class="deshboardtoplink">
		...
		<li class="last"><a href="javascript:void(0);"><?php echo $this->Html->image('front_end/blue_clip_img.jpg', array('alt'=>'', 'onclick'=>'return manage_feeds_tabs("business_feeds_link_container");', 'title'=>'Link'));?></a></li>

		<?php if($businessPlan > 2){ //for premium+ and platinum plans
			if($this->Fused->fetchMarkettingAvailable($businessArr['Business']['id'], $businessPlan) == 'yes'){
		?>
		<li class="last">
			<div style="width:280px;" id="marketting_div">
				<div style="float:right;">
					<input name="" id="marketting" type="checkbox" value="marketting" onclick="return validateMarketting();"> Marketting
				</div>
				<div class="clr"></div>
			</div>
		</li>
...
<!-- HIDDEN FIELDS START -->
<input type="hidden" id="marketting_feed" value="0"/>
<!-- HIDDEN FIELDS END   -->
...
function validate_business_post(){
	var flag = '0';
	var marketting = $('#marketting_feed').val();
...
function validateFeedsPostingSocialNetworks(socialMedias){
	...
	if(flag == '1'){
		...
		$.ajax({
			...
			data: "business_id="+business_id+"&image="+image+"&video="+video+"&link="+link+"&message="+message+"&social_medias="+social_medias+"&marketting="+marketting+"&image_caption="+image_caption+"&video_caption="+video_caption+"&link_caption="+link_caption+"&youtube_link="+youtube_link+"&yotube_type="+yotube_type,
			beforeSend:function(){
			...
			success: function(response){ //alert(response); return false;
				...
				if(spltArr[0] == 'saved'){
					...
					$('#marketting_feed').val('0');// set the value of the marketting to zero
					$('#marketting').removeAttr('checked'); //remove the checkbox
				}else
...
function fetchAjaxFeedsData(id){
	$.ajax({
		...
		success: function(response){
			...
			validateMarkettingField();
		}
	});
}

function validateMarkettingField(){
	$.ajax({
		type: "POST",
		url: "<?php echo SITE_PATH.'business_feeds/validate_marketting_feeds/';?>",
		data: "busines_id=<?php echo $businessArr['Business']['id'];?>&business_plan=<?php echo $businessPlan;?>",
		success: function(response){
			if(response == 'no'){
				$('#marketting_div').remove();
			}
		}
	});
}
...
<script type="text/javascript">
function validateMarketting(){
	var feed = 0;
	if($('#marketting').is(':checked')){
		feed = 1;
	}
	$('#marketting_feed').val(feed);
}
