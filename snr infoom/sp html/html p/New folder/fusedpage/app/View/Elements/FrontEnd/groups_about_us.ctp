<?php $groupAboutArr = $this->Fused->groupUserAboutUs(base64_decode($this->params['pass'][0])); ?>
<div id="pulic_2" class="businessintab" style="display:none;">			
		<h5>About us
		<?php if($grpArr['Group']['user_id'] == $this->Session->read('Auth.User.User.id')){?>
		<div class="editinlink">
			<a href="javascript:void(0);" onclick="return validateAboutsUsTabs();">Edit</a>
		</div>
		<div class="clr"></div>
		<?php } ?>
		</h5>
		...
		<div id="original_aboutUs_content"><?php echo nl2br($groupAboutArr);?></div>
		<!-- ORIGINAL CONTENT END -->

		<!-- UPDATE CONTENT START -->
		<div id="update_aboutUs_content" style="display:none;">
			<div>
				<?php echo $this->Form->textarea('Group.description', array('rows'=>25, 'cols'=>80, 'value'=>$groupAboutArr));?>
			</div>
			<div class="clr"></div>
...
<?php if($this->Fused->groupUserId() == base64_decode($this->params['pass'][0])){?>
...
function validateAboutUsContent(){
	var aboutUsMessage = $('#GroupDescription').val();
	if(aboutUsMessage != ''){
		//send Ajax for saving the data
		$.ajax({
			type: "POST",
			url: "<?php echo SITE_PATH.'groups/post_about_us_content/';?>",
			data: "aboutUsMessage="+aboutUsMessage+"&group_id=<?php echo base64_decode($this->params['pass'][0]);?>",
			beforeSend:function(){
