	...
	new AjaxUpload(button, {
		...
		onComplete: function(file, response){ //alert(file); return false;
			...
			if(splittedArr['1'] != ''){
				...
				var fileExpArr = file.split('.');
				var fileExp = fileExpArr[(fileExpArr.length) - 1];
				if((fileExp == 'jpg') || (fileExp == 'png') || (fileExp == 'jpeg') || (fileExp == 'gif')){ //for image
					var attachedFile = '<div class="mailbtmbox1"><img src="<?php echo SITE_PATH;?>img/front_end/attachments/'+splittedArr[2]+'" style="height:50px; width:50px;"/>'+splittedArr['1']+'</div>';				
				}else{
				...
				$('#uploaded_attachments').append(attachedFile); //show the uploaded file
			}else{
	...
	if(receiver_id != '' && message != ''){
		$.ajax({
			...
			success: function(response){
				...
				$('#message').val('');
				$('#main_messages').prepend(response);
				$('#scrollbar2').tinyscrollbar_update();
			}
		});
	}else
