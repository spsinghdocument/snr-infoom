<?php //pr($businessArr);
	...
	$userAdd = '';
	if($this->Session->read('Auth.User.User.city') != '')
		$userAdd .= $this->Session->read('Auth.User.User.city');
	if($this->Session->read('Auth.User.User.state') != '')
		$userAdd .= ', '.$this->Session->read('Auth.User.User.state');
	if($this->Session->read('Auth.User.User.country') != ''){
		if($this->Session->read('Auth.User.User.country') == 'CAN')
			$userAdd .= ', Canada';
		else
			$userAdd .= ', '.$this->Session->read('Auth.User.User.country');
	}
?>
<style type="text/css">
...
<div id="pulic_7" style="display:none;">
	<div class="deshlistbox">
		<div>
			<div>
				<div class="feddimgname" style="float:left; width:80px;">My location:</div>
				<div style="float:left; margin-left:5px;">
					<?php echo $this->Form->text('start', array('div'=>false, 'label'=>false, 'class'=>'mapClassInput', 'value'=>$userAdd));?>
				</div>
				<div class="clr"></div>
			</div>
			<div class="clr"></div>
