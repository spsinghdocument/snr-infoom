...
<div id="offer_add_category" style="display:none;">
	...
	<div class="contectflbox" style="width:400px;">
		...
		<div class="contactfield" style="float:left;">
			...
			<div id="categoryNameError" style="color:#FF0000;"></div>
		</div>
		<div class="clr"></div>
		...
		<div class="contactfield" style="padding-top:10px; float:left;">
			...
			<div id="add_category_post" style="float:left; margin-top:5px; text-align:center; width:190px; font-size:12px;">
			</div>
		</div>
		<div class="clr"></div>
	</div>
...
<div id="offer_add_container" style="display:none;">
	...
	<div class="contectflbox" style="width:400px;">
		<div class="contactlable" style="width:85px;">Name:</div>
		<div class="contactfield" style="float:left;">
			<?php echo $this->Form->select('BusinessOffer.name', $this->Fused->fetchOfferCategories(), array('empty'=>'Select', 'class'=>'contactinput addOfferClass', 'style'=>'width:310px;'));?>
		...
		<div class="contactfield" style="float:left;">
			<?php echo '$ '.$this->Form->text('BusinessOffer.price', array('div'=>false, 'label'=>false, 'class'=>'contactinput_1 addOfferClass'));?>
...
function validateOffersTabs(type){
	if(type == 'category'){
		...
		$('#add_category_post').html('');

		$('#offer_add_category').show();
		$('#OffersTabNavigate').html('<a href="javascript:void(0);" onclick="return validateOffersTabs(\'add\');">Add New Offer</a><a href="javascript:void(0);" onclick="return validateOffersTabs(\'list\');">List Offer</a>');
	}else if(type == 'add'){
		$('#resultAddOfferDiv').html(''); //reset main ajax div
		$('#showuploading').html(''); // reset image uploading div
		$('#main_offers_listing_container').hide();
		...
		$('#OffersTabNavigate').html('<a href="javascript:void(0);" onclick="return validateOffersTabs(\'category\');">Add "What Offered" Category</a><a href="javascript:void(0);" onclick="return validateOffersTabs(\'add\');">Add New Offer</a>');
	}else{
		$('#main_offers_listing_container').hide();
		$('#offer_edit_container').show();
		$('#OffersTabNavigate').html('<a href="javascript:void(0);" onclick="return validateOffersTabs(\'category\');">Add "What Offered" Category</a><a href="javascript:void(0);" onclick="return validateOffersTabs(\'list\');">List Offers</a><a href="javascript:void(0);" onclick="return validateOffersTabs(\'add\');">Add New Offer</a>');
...
function validateAddOfferForm(){
	...
	var BusinessOfferName = $('#BusinessOfferName').val(); //alert(BusinessOfferName); return false;
...
function validateEditOfferForm(){
	...
	var dataToSend = "type=save&id="+BusinessOfferIdEdit+"&name="+BusinessOfferNameEdit+"&title="+BusinessOfferTitleEdit+"&description="+BusinessEnquiryDescriptionEdit+"&price="+BusinessOfferPriceEdit+"&newImage="+BusinessOfferImageEdit.value;
	//alert(dataToSend); return false;
...
function validateAddCategoryForm(){
	...
	var category = $('#BusinessOfferCategory').val();
	if(category != ''){
		$.ajax({
			type: "POST",
			...
			beforeSend:function(){
				...
				$('#add_category_post').html(bSend);
			},
			success: function(response){
				if(response == 'saved'){
					$('#add_category_post').html('<font color="green">Category Added Successfully!!</font>');
					setTimeout(function(){
					      validateOffersTabs('list');
					}, 2000);
					...
					$('#add_category_post').html(response);
					return false;	
				}
			}
		});
	}else{
		$('#categoryNameError').html('Required');
		return false;
	}
}
</script>
