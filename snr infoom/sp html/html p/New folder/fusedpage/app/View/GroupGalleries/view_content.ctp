...
if($type == 'v'){ //For Video
	//validate Video
	$videoUrl = '../webroot/img/front_end/groups/flv/'.$businessArr['GroupGallery']['video'];
	if(is_file($videoUrl)){
		echo $this->Html->script('flowplayer/flowplayer-3.2.6.min');
		$player = SITE_PATH.'js/flowplayer/swf/flowplayer-3.2.7.swf';

		$playerVideo = SITE_PATH.'img/front_end/groups/flv/'.$businessArr['GroupGallery']['video'];

		//validate the Video Image
		$vidImg = str_replace('.flv', '.jpg', $businessArr['GroupGallery']['video']);
		$videoImageUrl = '../webroot/img/front_end/groups/flv/images/'.$vidImg;
		if(is_file($videoImageUrl))
			$playerVideoImage = SITE_PATH.'img/front_end/groups/flv/images/'.$vidImg;
		else
			$playerVideoImage = SITE_PATH.'img/front_end/groups/noimage.jpg';
?>
...
<?php
	...
	//FOR IMAGE START
	$imageUrl = '../webroot/img/front_end/groups/'.$businessArr['GroupGallery']['image'];
	if(is_file($imageUrl)){
		$imagePath = 'front_end/groups/'.$businessArr['GroupGallery']['image'];		
?>
