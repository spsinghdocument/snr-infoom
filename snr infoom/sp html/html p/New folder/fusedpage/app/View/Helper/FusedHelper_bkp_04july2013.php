...
class FusedHelper extends AppHelper {
	...
	public function fetchUsersRecentFeeds($profile_id=null){
		...
		//FOR FRIENDS START
		$friendsArr = $this->Friend->find('all', array('fields'=>array('Friend.request_sent','Friend.request_received'), 'conditions'=>array('Friend.friendship_status'=>'1', 'OR'=>array('Friend.request_sent'=>$user_id, 'Friend.request_received'=>$user_id)))); //pr($friendsArr);die;
		...
		$friend_id = array_merge($request_sent, $request_received);// pr($friend_id);die;
		...
		$businessArr = $this->Business->find('all', array('conditions'=>array('Business.user_id'=>$business_user_id, 'Business.status'=>'1')));
		$Userbusiness_id = '';
		if(!empty($businessArr)){
		...
		$feedsArr = $this->BusinessFeed->find('all', array('conditions'=>array('OR'=>array('BusinessFeed.user_id'=>$business_user_id, 'BusinessFeed.group_user_id'=>$business_user_id, 'BusinessFeed.business_id'=>$businessfeed_id, 'BusinessFeed.business_id'=>$Userbusiness_id, 'BusinessFeed.marketting'=>'1'), 'BusinessFeed.status'=>'1'), 'limit'=>PAGING_SIZE, 'order'=>array('BusinessFeed.id'=>'DESC')));
			...
			//$businessfeed_id = '';
			$ret = '';
			$feedsArr = $this->BusinessFeed->find('all', array('conditions'=>array('OR'=>array('BusinessFeed.user_id'=>$user_id, 'BusinessFeed.business_id'=>$business_id, 'BusinessFeed.business_id'=>$businesssubscriberfeed_id, 'BusinessFeed.marketting'=>'1'), 'BusinessFeed.status'=>'1'), 'limit'=>PAGING_SIZE, 'order'=>array('BusinessFeed.id'=>'DESC')));
		}
		//pr($feedsArr); die;
		if(!empty($feedsArr))
			$ret = $feedsArr;
...
/*------------------ 21 MAY 2013 ------------------------------*/
	...
	public function validateTheUserFeed($visibility, $profile=null, $feed_user_id){ //echo $visibility.', '.$profile.', '.$feed_user_id;die;
