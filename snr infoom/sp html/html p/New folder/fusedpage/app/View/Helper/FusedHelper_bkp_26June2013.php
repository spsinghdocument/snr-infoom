...
class FusedHelper extends AppHelper {
	...
	public function fetchUsersRecentFeeds($profile_id=null){
		...
		$feedsArr = $this->BusinessFeed->find('all', array('conditions'=>array('OR'=>array('BusinessFeed.user_id'=>$business_user_id, 'BusinessFeed.group_user_id'=>$business_user_id, 'BusinessFeed.business_id'=>$businessfeed_id, 'BusinessFeed.business_id'=>$Userbusiness_id, 'BusinessFeed.marketting'=>'1'), 'BusinessFeed.status'=>'1'), 'limit'=>PAGING_SIZE, 'order'=>array('BusinessFeed.id'=>'DESC')));
			...
			$businessfeed_id = '';
			$ret = '';
			$feedsArr = $this->BusinessFeed->find('all', array('conditions'=>array('OR'=>array('BusinessFeed.user_id'=>$user_id, 'BusinessFeed.business_id'=>$businessfeed_id, 'BusinessFeed.marketting'=>'1'), 'BusinessFeed.status'=>'1'), 'limit'=>PAGING_SIZE, 'order'=>array('BusinessFeed.id'=>'DESC')));
		}
		//pr($feedsArr); die;
		if(!empty($feedsArr))
			$ret = $feedsArr;
...
/*------------------ 21 MAY 2013 ------------------------------*/
	...
	public function fetchHIWPageContent($type){
		...
		$headingArr = $this->HowItWork->find('all', array('fields'=>array('HowItWork.id', 'HowItWork.heading', 'HowItWork.content', 'HowItWork.image'), 'conditions'=>array('HowItWork.status'=>'1', 'HowItWork.type'=>$type, 'HowItWork.id >'=>'2'), 'order'=>array('HowItWork.id'=>'DESC')));
		return $headingArr;
	}
