		...
		if(!empty($membershipArr)){
			...
			if(!empty($businessArr)){
				if($businessArr['Business']['user_id'] == ''){
					...
					$address2 = urlencode('');
					$city = urlencode($businessArr['Business']['city']);
					$state = urlencode($businessArr['Business']['state']);
					$zip = urlencode($businessArr['Business']['zip']);

					//Load The Country Model
					Controller::loadModel('Country');
					$countryArr = $this->Country->find('first', array('fields'=>array('Country.country_iso_code_2'), 'conditions'=>array('Country.country_iso_code_3'=>$businessArr['Business']['country'])));
