...
<script type="text/javascript">	
	$(document).ready(function(){
		$("#BusinessDealAdminEditForm").validationEngine()	
	});	
</script>
...
<div class="rightMid">
	...
	<!-- DISPLAY MESSAGE END -->
	<div class="mainHd">Edit Deal</div>

	<div>
	<?php 
		echo $this->Form->create('BusinessDeal', array('action'=>'admin_edit', 'type'=>'file'));
		echo $this->Form->hidden('BusinessDeal.id');
		echo $this->Form->hidden('BusinessDeal.business_id');
	?>
		<div class="formField">
			<span>Title :</span>
		...
		<div class="formField">
			...
			echo $this->Form->hidden('BusinessDeal.old_image', array('value'=>$this->data['BusinessDeal']['image']));

			if($this->data['BusinessDeal']['image'] != ''){
				$imagerealPath = '../webroot/img/front_end/business/deals/'.$this->data['BusinessDeal']['image'];
				if(is_file($imagerealPath)){
			?>
			<span style="float:right; margin-right:300px;">
				<?php echo $this->Image->resize('front_end/business/deals/'.$this->data['BusinessDeal']['image'], 50, 50, array('alt'=>''));?>
			</span>
			<?php
				}
			}
