...
<!-- Recommended Start -->
				...
				<div class="likeboxfl" id="image1_<?php echo $offer['BusinessOffer']['id']; ?>">	
				<?php
...
<?php $countUserImage = $this->Fused->fetchUserOffer($offer['BusinessOffer']['id'],$this->Session->read('Auth.User.User.id')); ?>
				...
				<span id="like_<?php echo $offer['BusinessOffer']['id']; ?>">
				...
				<?php } ?>
					<span id="recommended_<?php echo $offer['BusinessOffer']['id'];?>"> <?php if($countRecommended > 0){ echo $countRecommended; } else { echo '0'; } ?> People</span> recommended this
				</div>
				<span id="roller1_<?php echo $offer['BusinessOffer']['id'];?>"></span>
				<div class="clr"></div>
<!-- Recommended End -->
...
<!-- Comment Start -->
				<div class="coomentlinbox" id="comment_div_<?php echo $offer['BusinessOffer']['id'];?>" style="border:none;">
				...
				<span id="roller_<?php echo $offer['BusinessOffer']['id'];?>"></span>
<!-- Comment End -->
	...
	if(comment != ''){
		$.ajax({
				...
				beforeSend:function(){
					...
					$('#roller_'+offer_id).html(bSend);
				},
				success: function(response){
				alert(response);
						$('#roller_'+offer_id).html('');
						$('#offer_comment_'+offer_id).val('');
						$('#comment_div_'+offer_id).append(response);
...
var user_id = "<?php echo $this->Session->read('Auth.User.User.id'); ?>";
	if(user_id != ''){
		$.ajax({
				...
				beforeSend:function(){
					var bSend = '<?php echo $this->Html->image("ajax/small-facebook.gif", array("alt"=>""));?>';
					$('#roller1_'+offer_id).html(bSend);
				},
				success: function(response){
						$('#roller1_'+offer_id).html('');
						$('#recommended_'+offer_id).html(response);
						$('#like_'+offer_id).hide();
	...
	if(user_id != ''){
		$.ajax({
				...
				success: function(response){
						...
						$('#image1_'+offer_id).append(response);
				}
			});
	}
}
