<div id="pulic_3" style="display:none;">
	...
	<?php if($this->Fused->validateUserForBusiness() == $this->Fused->decrypt($this->params['pass'][0])){?>
		<div class="editinlink" id="OffersTabNavigate">
			<a href="javascript:void(0);" onclick="return validateOffersTabs('category');" style="display:inline;">Add "What Offered" Category</a>
			<a href="javascript:void(0);" onclick="return validateOffersTabs('add');" style="display:inline;">Add an Offer</a>
		</div>
		<div class="clr"></div>
	<?php } ?>
	</h5>
...
<div id="offer_add_category" style="display:none;">
	...
	<div class="contectflbox" style="width:400px;">
		...
		<div class="contactfield" style="float:left;">
			<?php echo $this->Form->text('BusinessOffer.category', array('div'=>false, 'label'=>false, 'class'=>'contactinput addOfferClass', 'maxlength'=>200));?>
		...
		<div class="contactfield" style="padding-top:10px; float:left;">
			<div style="float:left; width:130px;">
				<?php echo $this->Form->submit('front_end/submit_btn.png', array('div'=>false, 'onclick'=>'validateAddCategoryForm();'));?>
			</div>
			<div id="add_enquiry_error" style="float:left; margin-top:5px; text-align:center; width:190px; font-size:12px;">
			</div>
		</div>
		<div class="clr"></div>
	</div>
	...
	<div class="contectflbox" style="width:400px;">
		...
		<div class="contactfield" style="float:left;">
			<?php //echo $this->Form->text('BusinessOffer.name', array('div'=>false, 'label'=>false, 'class'=>'contactinput addOfferClass', 'maxlength'=>50));
				echo $this->Form->select('BusinessOffer.name', $this->Fused->fetchOfferCategories(), array('empty'=>'Select', 'class'=>'contactinput addOfferClass', 'style'=>'width:310px;'));
			?>
			<div id="offerNameError" style="color:#FF0000;"></div>
		</div>
		<div class="clr"></div>
...
function validateOffersTabs(type){
	if(type == 'category'){
		...
		$('#OffersTabNavigate').html('<a href="javascript:void(0);" onclick="return validateOffersTabs(\'list\');" style="display:inline;">List Offer</a>');
	}else if(type == 'add'){
		$('#resultAddOfferDiv').html(''); //reset main ajax div
		$('#showuploading').html(''); // reset image uploading div
		$('#main_offers_listing_container').hide();
		...
		$('#OffersTabNavigate').html('<a href="javascript:void(0);" onclick="return validateOffersTabs(\'category\');" style="display:inline;">Add "What Offered" Category</a><a href="javascript:void(0);" onclick="return validateOffersTabs(\'list\');" style="display:inline;">List Offer</a>');
	}else if(type == 'list'){
		$('#offer_add_container').hide();
		$('#offer_edit_container').hide();
		$('#main_offers_listing_container').show();
		$('#offer_add_category').hide();
		$('#OffersTabNavigate').html('<a href="javascript:void(0);" onclick="return validateOffersTabs(\'category\');" style="display:inline;">Add "What Offered" Category</a><a href="javascript:void(0);" onclick="return validateOffersTabs(\'add\');" style="display:inline;">Add New Offer</a>');
	}else{
		$('#main_offers_listing_container').hide();
...
function validateEditOfferForm(){
	...
	var newImage = $('#BusinessOfferImageEdit').val();
	var oldImage = $('#BusinessOfferOldImage').val();
	...
	var dataToSend = "type=save&id="+BusinessOfferIdEdit+"&name="+BusinessOfferNameEdit+"&title="+BusinessOfferTitleEdit+"&description="+BusinessEnquiryDescriptionEdit+"&price="+BusinessOfferPriceEdit+"&newImage="+newImage+"&oldImage="+oldImage;
...
function validateAddCategoryForm(){
	var category_name = $('#BusinessOfferCategory').val();
	if(category_name != ''){
		$.ajax({
			type: "POST",
			url: "<?php echo SITE_PATH.'business_offers/add_category/';?>",
			data: "category="+category_name,
			beforeSend:function(){
				var bSend = '<div style="text-align:center; color:#0171C3; margin-top:50px;"><?php echo $this->Html->image("ajax/pic-loader.gif", array("alt"=>""));?></div>';
				$('#add_enquiry_error').html(bSend);
			},
			success: function(response){
				if(response == 'saved'){
					$('#BusinessOfferCategory').val('');
					$('#add_enquiry_error').html('<font color="green">Category Saved Successfully!!</font>');
				}else
					$('#add_enquiry_error').html(response);
			}
		});
