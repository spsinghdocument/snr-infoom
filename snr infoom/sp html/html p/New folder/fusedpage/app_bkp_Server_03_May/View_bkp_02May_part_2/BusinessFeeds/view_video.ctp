...
//validate Video
$videoUrl = '../webroot/img/front_end/business/feeds/video/flv/'.$video.'.flv';
if(is_file($videoUrl)){
	$playerVideo = SITE_PATH.'img/front_end/business/feeds/video/flv/'.$video.'.flv';

	//validate the Video Image
	$videoImageUrl = '../webroot/img/front_end/business/feeds/video/image/'.$video.'.jpg';
	if(is_file($videoImageUrl))
		$playerVideoImage = SITE_PATH.'img/front_end/business/feeds/video/image/'.$video.'.jpg';
	else
...
<?php }
	...
	$imageRealPath = '../webroot/img/front_end/business/feeds/'.$imageName;

	if(is_file($imageRealPath)){
		echo $this->Html->image('front_end/business/feeds/'.$imageName, array('alt'=>''));
	}else{
?>
