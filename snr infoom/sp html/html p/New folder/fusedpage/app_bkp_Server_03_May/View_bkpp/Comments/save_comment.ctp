...
<tr>
	...
	<td><?php echo $comment;?><?php if($this->Session->read('Auth.User.User.id') == $userr_id){ ?>
					<div class="listboxbg"><span style="float:right; margin-top:-10px;" id="delete_comment_<?php echo $last_id;?>"><a href="javascript:void(0);" title="Delete" onclick="return deleteCommentFeed('<?php echo $last_id;?>');"><?php echo $this->Html->image('front_end/delete.png', array('alt'=>'', 'style'=>'width:16px; height:16px;'));?></a></span></div>
					<?php } ?></td>
</tr>
</table>